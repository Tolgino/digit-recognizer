import pygame
import numpy as np
import digitrecognizer as digitrecognizer
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_SPACE:
                ...
                image_array = np.zeros((board_height, board_width), dtype=np.uint8)
                predicted_digit = None  # Reset the predicted digit
            elif event.key == pygame.K_RETURN:
                # Predict the digit when the enter key is pressed
                model = digitrecognizer.load_model()
                ...
                if len(drawn_images) == 0:
                    ...
                    for img in drawn_images:
                        ...
                        grayscale_array = np.transpose(grayscale_array)

                        # Predict the digit using the model
                        prediction = digitrecognizer.predict_digit(model, grayscale_array)
                        predicted_digits.append(str(prediction))

                    predicted_digit = "".join(predicted_digits)  # Update the predicted digits
    ...
    text_surface = font.render("Predicted number: " + (predicted_digit if predicted_digit is not None else ""), True, (0, 0, 0))  # Render text in black
    canvas.blit(text_surface, (board_width + 10, board_height // 2))

    # Render instructions
    instructions_text = font.render("Press Enter to calculate", True, (0, 0, 0))
